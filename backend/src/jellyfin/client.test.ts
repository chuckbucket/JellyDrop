...
describe("streamTranscodedProxy", () => {
  ...
  it("requests Jellyfin's transcode-stream endpoint with explicit width+height (not maxHeight — confirmed live that hardware transcoding ignores max-bound params)", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(null, { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);

    await jellyfinClient.streamTranscodedProxy("item-1", 1280, 720, 2_500_000);
    ...
    expect(url.searchParams.has("maxHeight")).toBe(false);
    expect(url.searchParams.has("maxWidth")).toBe(false);
  ...
  it("uses a fresh playSessionId per call", async () => {
    ...
    await jellyfinClient.streamTranscodedProxy("item-1", 640, 480, 1_200_000);
    await jellyfinClient.streamTranscodedProxy("item-1", 640, 480, 1_200_000);
